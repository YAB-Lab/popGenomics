...
while ($a<$numfiles){
	...
	if($allfiles[$a] =~ /.fa$/){
		...
		# print "ID: $ID, SEQ: $SEQ, OutSEQ: $outSEQ\n";
		$/ = "\n";
		print "Number of outgroup sequences parsed: " . scalar(@{$outSEQ}) . "\n";
		if($outSEQ->[0] && scalar @{$outSEQ} == $num_outs && $SEQ->[1]){
			calcMK($SEQ, $outSEQ);
		} else {
			print " mismatch problem: outSEQ->[0]=$outSEQ->[0], num_outs=$num_outs, SEQ->[1]=$SEQ->[1]\n";
		}
...
sub parseINOUT{
	...
	while(my $data = <FASTAINPUT>){
		...
		# print "DEBUG: Parsing sequence for header: $header\n";

		if($tempSP eq $opt_ingroup){
			# print "DEBUG: Adding to ingroup: $header\n";
			$data =~ s/.*?\n//;	# remove defline
...
print "Debug: Writing output to file...\n";
		exit;
    }
}
