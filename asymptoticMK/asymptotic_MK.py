...
def filter_variants_coding(input_vcf, output_d, output_d0, output_p, output_p0, snpsift_path):
    """Extract fixed and segregating sites using SnpSift."""
    snpsift = os.path.join(snpsift_path, "SnpSift.jar")
    print("[INFO] Extracting variant types using SnpSift...")
    run_command(f"java -jar {snpsift} filter \"(ANN[*].EFFECT has 'missense_variant') && (AF = 1.0)\" {input_vcf} > {output_d}", "Filtering missense fixed variants")
    run_command(f"java -jar {snpsift} filter \"(ANN[*].EFFECT has 'synonymous_variant') && (AF = 1.0)\" {input_vcf} > {output_d0}", "Filtering synonymous fixed variants")
    run_command(f"java -jar {snpsift} filter \"(ANN[*].EFFECT has 'missense_variant') && (AF < 1.0)\" {input_vcf} > {output_p}", "Filtering missense segregating variants")
    run_command(f"java -jar {snpsift} filter \"(ANN[*].EFFECT has 'synonymous_variant') && (AF < 1.0)\" {input_vcf} > {output_p0}", "Filtering synonymous segregating variants")

def filter_variants_noncoding(input_vcf1, input_vcf2, output_d, output_d0, output_p, output_p0, snpsift_path):
    """Extract fixed and segregating sites using SnpSift."""
    snpsift = os.path.join(snpsift_path, "SnpSift.jar")
    print("[INFO] Extracting variant types using SnpSift...")
    run_command(f"java -jar {snpsift} filter \"AF = 1.0\" {input_vcf1} > {output_d}", "Filtering test region fixed variants")
    run_command(f"java -jar {snpsift} filter \"AF = 1.0\" {input_vcf2} > {output_d0}", "Filtering neutral fixed variants")
    run_command(f"java -jar {snpsift} filter \"AF < 1.0\" {input_vcf1} > {output_p}", "Filtering test region segregating variants")
    run_command(f"java -jar {snpsift} filter \"AF < 1.0\" {input_vcf2} > {output_p0}", "Filtering neutral segregating variants")
...
def main():
    ...
    "The asymptotic MK approach was developed by Messer and Petrov (2013) and the current implementation in R was developed by Haller and Messer (2017). ")
    ...
    parser.add_argument("-r", "--region", help="Region for filtering (optional; required if mode = 'noncoding')")
    parser.add_argument("-r2", "--region2", help="Second region for filtering in noncoding mode (required if mode = 'noncoding')")
    ...
    if args.mode == "coding":
        ...
        filter_variants_coding(filtered_vcf, results_d, results_d0, results_p, results_p0, args.snpsift_path)
        ...
        filter_variants_noncoding(filtered_vcf1, filtered_vcf2, results_d, results_d0, results_p, results_p0, args.snpsift_path)
    ...
    # filter_variants(filtered_vcf, results_d, results_d0, results_p, results_p0, args.snpsift_path)
    ...
    extract_allele_frequencies(results_p, f"{args.output_prefix}_freq_p.txt", args.snpsift_path)
    extract_allele_frequencies(results_p0, f"{args.output_prefix}_freq_p0.txt", args.snpsift_path)
