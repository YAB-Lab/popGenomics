...
suppressWarnings(suppressMessages(library(nls2)))
...
if (length(args) < 6) {
    stop("Usage: Rscript asymptoticMK_local.R <d0> <d> <xlow> <xhigh> <mk_table> <output_file>")
}

# Parse input arguments
d0 <- as.numeric(args[1])
d <- as.numeric(args[2])
xlow <- as.numeric(args[3])
xhigh <- as.numeric(args[4])
mk_table <- args[5]
output_file <- args[6]

# Read the mk_table file
df <- read.table(mk_table, header=TRUE, sep="\t")

# Ensure the dataframe has the required columns
if (!all(c("x", "p", "p0") %in% colnames(df))) {
    stop("Error: mk_table file must have columns 'x', 'p', and 'p0'")
}
...
createOutputDevice <- function(output, plotNum, width=3.5, height=3.5, bg="white", output_file, ...)
{
	titleString <- paste0(output_file, "_figure_", plotNum)
...
asymptoticMK <- function(d0, d, xlow, xhigh, df, true_alpha=NA, output="table", ...)
{
	...
	else
	{
		...
		#
		createOutputDevice(output=output, plotNum=1, output_file=output_file)
		...
		#
		createOutputDevice(output=output, plotNum=2, output_file=output_file)
		...
		#	PLOT 3: alpha(x) ~ x
		#
		createOutputDevice(output=output, plotNum=3, output_file=output_file)
		...
		#
		createOutputDevice(output=output, plotNum=4, output_file=output_file)
...
# Run the asymptoticMK function
result <- asymptoticMK(d0, d, xlow, xhigh, df, output="table")
asymptoticMK(d0, d, xlow, xhigh, df, output="pdf")

# Write the results to the output file
write.table(result, file=paste0(output_file, ".tsv"), sep="\t", quote=FALSE, row.names=FALSE)

cat("Asymptotic MK test results saved to", output_file, ".tsv\n")
